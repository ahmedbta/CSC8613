...
def evaluate_production(as_of: str, seed: int) -> dict:
    ...
    if not latest_prod:
        print("Aucun modèle en Production : le candidat sera promu par défaut.")
        return {
            "prod_version": "None",
            "prod_auc": None,
            "prod_f1": None,
...
def compare_and_promote(candidate: dict, production: dict, delta: float) -> str:
    ...
    prod_auc_str = f"{prod_auc:.4f}" if prod_auc is not None else "None"
    print(f"[COMPARE] candidate_auc={new_auc:.4f} vs prod_auc={prod_auc_str} (delta={delta:.4f})")
...
def train_and_compare_flow(as_of: str, seed: int = 42, delta: float = 0.01):
    ...
    prod_auc_summary = 'None' if prod['prod_auc'] is None else f"{prod['prod_auc']:.4f}"
    print(
        f"[SUMMARY] as_of={as_of} cand_v={cand['candidate_version']} "
        f"cand_auc={cand['val_auc']:.4f} prod_v={prod['prod_version']} prod_auc={prod_auc_summary} -> {decision}"
    )
    return decision
